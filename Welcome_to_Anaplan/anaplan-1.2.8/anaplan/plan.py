    ...
    for index, inp in enumerate(x_train):
        ...
        if val == True:
                ...
                if int(progress) % val_count == 1 and val_old_progress != int(progress):
                    ...
                    validation_model = evaluate(x_val, y_val, LTPW , loading_bar_status=False, acc_bar_status=False, activation_potentiation=activation_potentiation, show_metrices=None)
...
def activation_optimizer(x_train, y_train, x_val=None, y_val=None, depth=None, early_stop=False, show_history=False, interval=100, target_acc=None, decision_boundary_status=True, except_this=None, only_this=None):
    """
    Optimizes the activation functions for a neural network by leveraging validation data to find the most accurate 
    combination of activation functions for the given dataset. This function is designed to be used before model 
    training and allows for early stopping and decision boundary visualization.

    Args:
        x_train (array-like): Training input data.
        y_train (array-like): Labels for training data.
        x_val (array-like): Validation input data.
        y_val (array-like): Labels for validation data.
        depth (int, optional): The depth of the neural network. Default is None.
        ...
        decision_boundary_status (bool, optional): If True, visualizes the decision boundary after optimization. Default is True.
    ...
    Returns:
        tuple: A list for 'activation_potentiation' hyperparameter.
    """

    print(Fore.WHITE + "\nRemember, optimization on large datasets can be very time-consuming and computationally expensive. Therefore, if you are working with such a dataset, our recommendation is to include activation functions like ['spiral', 'circular'] in the 'except_this' parameter unless absolutely necessary, as they can significantly prolong the process. from: activation_optimizer\n" + Fore.RESET)
    ...
    if show_history == True:
        ...
        fig.suptitle('Activation Finder History')
        best_acc_list = []
        depth_list = []
        artist1 = []
        artist2 = []

    progress = tqdm(total=len(activation_potentiation),leave=False,ascii="▱▰",
    bar_format= bar_format_optimizer, ncols=60)

    acc_bar = tqdm(total=100,leave=False,ascii="▱▰",
    bar_format= bar_for_acc, desc='Test Accuracy', ncols=70)
    ...
    for i in range(depth):
        ...
        for j in range(len(activation_potentiation)):
            ...
            model = evaluate(x_val, y_val, W=W, acc_bar_status=False, loading_bar_status=False, activation_potentiation=activations)

            val_acc = model[get_acc()]

            if val_acc >= best_acc:

                best_acc = val_acc
                acc_bar.set_postfix({"Validation Accuracy": best_acc})
                ...
                if target_acc != None:
                    ...
                    if best_acc >= target_acc:
                        ...
                        print('\nBest Found Accuracy: {:.4f}'.format(best_acc))
                        print('Best Found Activations: ', activations)
            ...
            progress.update(1)
            progress.set_postfix({"Depth": str(i) + '/' + str(depth-1)})
        ...
        if show_history == True:
            ...
            if decision_boundary_status == True:
                ...
                art1_1, art1_2 = plot_decision_boundary(x_val, y_val, activation_potentiation, W, artist=artist1, ax=ax)
                artist1.append([*art1_1.collections, art1_2])

            art2 = ax[1].plot(depth_list, best_acc_list, color='g', markersize=6, linewidth=2, label='Accuracy over Depth')
            ax[1].set_title('Accuracy over Depth')
        ...
        if early_stop == True:
            ...
            if i > 0:
                ...
                if best_acc_list[i] == best_acc_list[i-1]:
                    ...
                    print('\nEarly Stoppping\n')
                    print('\nBest Found Accuracy: {:.4f}'.format(best_acc))
                    print('Best Found Activations: ', best_activations)

                    return best_activations

        if i == 0:

            best_activations = [current_best_activation]
...
def evaluate(
    ...
    try:
        ...
        if loading_bar_status == True:
            ...
            bar_format= bar_format, desc='Testing',ncols=100)
        
        if acc_bar_status == True:

            acc_bar = tqdm(total=1, ascii="▱▰",
            bar_format= bar_for_acc, ncols=100)
...
def predict_model_ssd(Input, model_name, model_path):
    ...
    activation_potentiation = model[get_pot()]
