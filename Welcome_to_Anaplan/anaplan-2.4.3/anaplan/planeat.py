...
def learner(weights, activation_potentiations, what_gen, y_reward, show_info=False, strategy='cross_over', policy='normal_selective', mutations=True, bad_genoms_mutation_prob=None, activation_mutate_prob=0.5, save_best_genom=True, cross_over_mode='tpm'):
   """
    ...
    Args:
        ...
        \n strategy (str, optional): The strategy for combining the best and bad genomes. Options: \n
                                  - 'cross_over': Perform Crossover between the best genomes and replace bad genomes. (Classic NEAT cross over)\n
        ...
        cross_over_mode (str): Two options: 'tpm'(Two-Point Matrix Crossover) or 'plan'(Output Connections Crossover) Default is: 'tpm'
        
    Raises:
        ValueError: 
            - If `policy` is not one of the specified values ('normal_selective', 'more_selective', 'less_selective').
            - If `cross_over_mode` is not one of the specified values ('tpm', 'plan').
...
### ERROR AND CONFIGURATION CHECKS:
   ...
   if cross_over_mode != 'tpm' and cross_over_mode != 'plan':
      raise ValueError("cross_over_mode parameter must be 'tpm' or 'plan'")
...
def cross_over(best_weight, good_weight, best_activations, good_activations, cross_over_mode='tpm'):
   """
      ...
      if mutation_action_prob < 0.33:
         ...
         if mutation_action_prob > 0.66 and len(activations) > 1:
            ...
            for i in range(weight.shape[0]):
               ...
               weight[i,:] = apply_activation(weight[i,:], activations[random_index_all_act])
