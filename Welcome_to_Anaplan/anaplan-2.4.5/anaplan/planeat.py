...
def learner(weights, activation_potentiations, what_gen, y_reward, show_info=False, strategy='cross_over', policy='normal_selective', mutations=True, bad_genoms_mutation_prob=None, activation_mutate_prob=0.5, save_best_genom=True, cross_over_mode='tpm', activation_add_prob=0.9, activation_delete_prob=0.1, activation_change_prob=0.5, weight_mutate_prob=1, weight_mutate_rate=32):
   """
...
Args:
    weights (numpy.ndarray): Array of weights for each genome. (first returned value of define_genomes function)
    activation_potentiations (list): A list of activation functions for each genome. (second returned value of define_genomes function)
    what_gen (int): The current generation number, used for informational purposes or logging.
    y_reward (numpy.ndarray): A 1D array containing the fitness or reward values of each genome. The array is used to rank the genomes based on their performance. PLANEAT maximizes the reward.
    show_info (bool, optional): If True, prints information about the current generation and the maximum reward obtained. Also shows the current configuration. Default is False.
    strategy (str, optional): The strategy for combining the best and bad genomes. Options:
                              - 'cross_over': Perform crossover between the best genomes and replace bad genomes. (Classic NEAT crossover)
                              - 'potentiate': Cumulate the weight of the best genomes and replace bad genomes. (PLAN feature, similar to arithmetic crossover but different.)
    ...
    bad_genoms_mutation_prob (float, optional): The probability of applying mutation to the bad genomes. Must be in the range [0, 1]. Also affects the mutation probability of the best genomes inversely. For example, a value of 0.7 for bad genomes implies 0.3 for best genomes. Default is None, which means it is determined by the `policy` argument.
    ...
    save_best_genom (bool, optional): If True, ensures that the best genomes are saved and not mutated or altered during reproduction. Default is True.
    cross_over_mode (str, optional): Specifies the crossover method to use. Options:
                                      ...
                                      Default is 'tpm'.
    activation_add_prob (float, optional): The probability of adding a new activation function to the genome. Must be in the range [0, 1]. Default is 0.9.
    activation_delete_prob (float, optional): The probability of deleting an existing activation function from the genome. Must be in the range [0, 1]. Default is 0.1.
    activation_change_prob (float, optional): The probability of changing an activation function in the genome. Must be in the range [0, 1]. Default is 0.5.
    weight_mutate_prob (float, optional): The probability of mutating a weight in the genome. Must be in the range [0, 1]. Default is 1.
    weight_mutate_rate (int, optional): If the value you enter here is equal to the result of input layer * output layer, only a single weight will be mutated during each mutation process. If the value you enter here is half of the result of input layer * output layer, two weights in the weight matrix will be mutated. WARNING: if you don't understand do NOT change this value. Default is 32.
...
### ERROR AND CONFIGURATION CHECKS:
   ...
   if (activation_add_prob < 0 or activation_add_prob > 1) or (activation_change_prob < 0 or activation_change_prob > 1) or (activation_delete_prob < 0 or activation_delete_prob > 1) or (weight_mutate_prob < 0 or weight_mutate_prob > 1):
...
### NEAT IS APPLIED ACCORDING TO THE SPECIFIED POLICY, STRATEGY, AND PROBABILITY CONFIGURATION:
   ...
   for i in tqdm(range(len(bad_weights)), desc="GENERATION: " + str(what_gen), bar_format=bar_format, ncols=50, ascii="▱▰"):
      ...
      if policy == 'normal_selective':
         ...
         if strategy == 'cross_over':
            bad_weights[i], bad_activations[i] = cross_over(best_weight, best_weights[i], best_activations=best_activation, good_activations=best_activations[i], cross_over_mode=cross_over_mode)
      ...
      if policy == 'less_selective':
            ...
            if strategy == 'cross_over': 
               bad_weights[i], bad_activations[i] = cross_over(best_weights[random_index], best_weights[i], best_activations=best_activations[random_index], good_activations=best_activations[i], cross_over_mode=cross_over_mode)
   ...
   if show_info == True:
      ...
      print("  WEIGHT MUTATE PROB: ", str(round(weight_mutate_prob, 2)))
      print("  WEIGHT MUTATE RATE (THRESHOLD VALUE FOR SINGLE MUTATION): ", str(weight_mutate_rate))
      print("  ACTIVATION MUTATE PROB: ", str(round(activation_mutate_prob, 2)))
      print("  ACTIVATION ADD PROB: ", str(round(activation_add_prob, 2)))
      print("  ACTIVATION DELETE PROB: ", str(round(activation_delete_prob, 2)))
      print("  ACTIVATION CHANGE PROB: ", str(round(activation_change_prob, 2)) + '\n')
...
def cross_over(best_weight, good_weight, best_activations, good_activations, cross_over_mode):
   """
