...
def fit(
    ...
    fit Args:
        ...
        loading_voice(bool, optional): Loading speak may effects training time (True or False) (optional) default: False
        ...
        val_bar_status(bool): Validating loading bar? True or False. Default is True. (optional)
        is_normalization(bool): Normalization process during training. May effect training time and model quality. True or False. Default is True. (optional)
    ...
    for index, inp in enumerate(x_train):
        ...
        if val == True:
                ...
                if int(progress) % val_count == 1 and val_old_progress != int(progress):
                    ...
                    validation_model = evaluate(x_test, y_test, LTPW , loading_bar_status=False, acc_bar_status=False, activation_potentiation=activation_potentiation, show_metrics=None)
                    ...
                    if show_training == True:
                        ...
                        for i in range(LTPW.shape[0]):
                            for j in range(LTPW.shape[1]):
                                if LTPW[i, j] != 0:
                                    G.add_edge(f'Motor Neuron{i}', f'Sensory Neuron{j}', ltpw=LTPW[i, j])
...
def neuron_history(LTPW, ax1, row, col, class_count, artist5, fig1, acc=False, loss=False):
    ...
    for j in range(len(class_count)):
        ...
        if row != 0:
            ...
            if acc != False and loss != False:
                suptitle_info = 'Test Accuracy: ' + str(acc) + '\nTest Loss: ' + str(loss) + '\nNeurons Memory:'
            else:
            ...
            mat = LTPW
            art5 = ax1.imshow(mat, interpolation='sinc', cmap='viridis')

            if acc != False and loss != False:
                suptitle_info = 'Test Accuracy: ' + str(acc) + '\nTest Loss: ' + str(loss) + '\nWeight Matrix Memory (Neurons Memory Cannot Be Visualized):'
            else:
                suptitle_info = 'Weight Matrix Memory (Neurons Memory Cannot Be Visualized):'
...
def learner(x_train, y_train, x_test=None, y_test=None, strategy='accuracy', big_data_mode=False, neurons_history=False, patience=None, depth=None, early_shifting=False, early_stop=False, loss='categorical_crossentropy', show_history=False, interval=33.33, target_acc=2, target_loss=-1, except_this=None, only_this=None):
    ...
    Args:
        ...
        strategy (str): Learning strategy. (options: 'accuracy', 'loss', 'adaptive_accuracy', 'adaptive_loss'): 'accuracy', Maximizes test accuracy during learning. 'loss', Minimizes test loss during learning. 'adaptive_accuracy', The model compares the current accuracy with the accuracy from the past based on the number specified by the patience value. If no improvement is observed it adapts to the condition by switching to the 'loss' strategy quickly starts minimizing loss and continues learning. 'adaptive_loss',The model adopts the 'loss' strategy until the loss reaches or falls below the value specified by the patience parameter. However, when the patience threshold is reached, it automatically switches to the 'accuracy' strategy and begins to maximize accuracy. Default is 'accuracy'.
        patience (int, float): patience value for adaptive strategies. For 'adaptive_accuracy' Default value: 5. For 'adaptive_loss' Default value: 0.150.
        depth (int, optional): The depth of the PLAN neural networks Aggreagation layers.
        big_data_mode (bool): Big data mode makes more faster training times and much better accuracy performance for big datasets. But in small datasets its opposide this. Makes negative impacts. Default is False
        ...
        target_acc (int, optional): The target accuracy to stop training early when achieved. Default is 2 (off).
        target_loss (float, optional): The target loss to stop training early when achieved. Default is -1 (off).
    ...
    for i in range(depth):
        ...
        for j in range(len(activation_potentiation)):
            ...
            W = fit(x_train, y_train, activation_potentiation=activations, train_bar=False, is_normalization=is_normalization)
            ...
            if (strategy == 'accuracy' and acc >= best_acc) or (strategy == 'loss' and test_loss <= best_loss):
                ...
                if neurons_history == True:
                    ...
                    artist4 = neuron_history(np.copy(best_weights), ax1, row, col, y_train[0], artist4, fig1, acc=best_acc, loss=test_loss)
