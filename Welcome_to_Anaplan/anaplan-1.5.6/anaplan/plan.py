...
def learner(x_train, y_train, x_test=None, y_test=None, strategy='accuracy', batch_size=1, show_current_activations=False, auto_normalization=True, neurons_history=False, patience=None, depth=None, early_shifting=False, early_stop=False, loss='categorical_crossentropy', show_history=False, interval=33.33, target_acc=2, target_loss=-1, except_this=None, only_this=None, start_this=None):
    ...
    if batch_size != 1:
        ...
        batch_acc_bar = tqdm(total=100, ascii="▱▰",
        bar_format= bar_format_learner_with_loss, ncols=91)
    ...
    for i in range(depth):
        ...
        for j in range(len(activation_potentiation)):
            ...
            model = evaluate(x_test_batch, y_test_batch, W=W, acc_bar_status=False, loading_bar_status=False, activation_potentiation=activations)
            ...
            if (strategy == 'accuracy' and acc >= best_acc) or (strategy == 'loss' and test_loss <= best_loss) or (strategy == 'f1' and f1_score >= best_f1) or (strategy == 'precision' and precision_score >= best_precision) or (strategy == 'recall' and recall_score >= best_recall) or (strategy == 'all' and f1_score >= best_f1 and acc >= best_acc and test_loss <= best_loss and precision_score >= best_precision and recall_score >= best_recall):
                ...
                if batch_size == 1:
                    ...
                    postfix_dict["Test Batch Accuracy"] = acc
                    batch_acc_bar.set_postfix(postfix_dict)
                    batch_acc_bar.update((100 * acc) - batch_acc_bar.n)
                ...
                if batch_size == 1:
                    ...
                    postfix_dict["Test Batch Loss"] = test_loss
                    batch_acc_bar.set_postfix(postfix_dict)
                    best_loss = test_loss
                ...
                if (target_acc is not None) or (target_loss is not None):
                    ...
                    if (best_acc >= target_acc) or (best_loss <= target_loss):
...
def encode_one_hot(y_train, y_test=None):
    """
